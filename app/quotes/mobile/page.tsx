...
import { ArrowRight, FileText, Plus, Save, Search, Smartphone } from "lucide-react";
...
export default function MobileQuotesPage() {
  ...
  function saveQuickDraft(event?: FormEvent) {
    ...
    if (!Number.isFinite(fixedPrice) || fixedPrice <= 0) { setError("Enter a valid fixed price."); return; }
  ...
  return <div className={`space-y-6 ${showQuickDraft ? "pb-32 lg:pb-0" : ""}`}>
    ...
    {showQuickDraft ? <Card><form onSubmit={saveQuickDraft} className="space-y-5">
      <div className="flex items-start gap-3"><Smartphone className="mt-0.5 size-5 text-cyan-300" /><div><h2 className="font-semibold">Fixed-price quick draft</h2><p className="mt-1 text-sm text-slate-400">Use this for a fast on-site draft. Internal labour and material costing can be added later in the full Quote Builder.</p></div></div>
      ...
      <InputField label="Customer fixed price (£)" inputMode="decimal" type="number" min="0.01" step="0.01" value={draft.fixedPrice} onChange={(event) => setDraft({ ...draft, fixedPrice: event.target.value })} />
    ...
    {showQuickDraft ? <MobileActionDock summary={<div><p className="truncate text-xs text-slate-400">Fixed-price draft</p><p className="truncate text-sm font-bold text-white">{draft.fixedPrice ? money.format(Number(draft.fixedPrice) || 0) : "Price not set"}</p></div>}>
