...
import { AlertTriangle, ArrowLeft, BriefcaseBusiness, CalendarCheck2, CheckCircle2, ClipboardCheck, FileText, ShieldCheck } from "lucide-react";
...
export default function JobReviewPage() {
  ...
  const activeJobs = jobs.items.filter((job) => job.status !== "Complete");
  const selected = activeJobs.find((job) => job.id === selectedId) || activeJobs[0];
  ...
  if (selected) {
    ...
    if (selected.status === "Complete" && !linkedCertificates.length) findings.push({ level: "Action", title: "Completed job has no certificate", detail: "Confirm whether certification is required and create or upload the final record.", href: "/certificates" });
    else if (linkedCertificates.length) findings.push({ level: "Ready", title: "Certificate records linked", detail: `${linkedCertificates.length} certificate record${linkedCertificates.length === 1 ? "" : "s"} found.` });
  }
  ...
  return <main className="space-y-6">
    <PageHeader
      eyebrow="JR AI"
      title="Job readiness review"
      description="Check whether a job has the commercial, scheduling, safety and site information needed before work starts."
