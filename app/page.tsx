...
export default function Home() {
  ...
  return <div className="space-y-6">
    ...
    <Card><h2 className="text-lg font-bold">Quick actions</h2><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Open Finance Director","/finance-director"],["Open payments","/payments"],["Prepare quote","/quotes"],["Open certificates","/certificates"]].map(([label,href]) => <Link key={href} href={href} className="flex items-center justify-between rounded-xl border border-slate-800 px-4 py-3 text-sm font-medium">{label}<ArrowRight className="size-4" /></Link>)}</div></Card>
  </div>;
}
