...
export default function Home() {
  ...
  return <div className="space-y-6">
    ...
    <Card><h2 className="text-lg font-bold">Quick actions</h2><p className="mt-1 text-sm text-slate-500">Jump straight into common work without loading extra dashboard modules.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Add customer","/customers"],["Create job","/jobs"],["Prepare quote","/quotes"],["Open certificates","/certificates"]].map(([label,href]) => <Link key={href} href={href} className="flex items-center justify-between rounded-xl border border-slate-800 px-4 py-3 text-sm font-medium text-slate-300 hover:border-slate-700 hover:bg-slate-800">{label}<ArrowRight className="size-4" /></Link>)}</div></Card>
  </div>;
}
