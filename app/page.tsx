...
export default function Home() {
  ...
  const dashboard = useMemo(() => {
    ...
    const expectedProfitRecords = activeProfitJobs.filter((job) => Boolean(job.quoteSnapshot?.profitability || linkedQuoteProfit.get(job.sourceQuoteId ?? ""))).length
      + wonQuotesAwaitingJob.filter((quote) => Boolean(quote.profitability)).length;
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-4">
      <div><p className="text-xs font-semibold uppercase tracking-wider text-cyan-400">Workflow performance</p><h2 className="mt-1 text-2xl font-bold">Quote-to-payment dashboard</h2><p className="mt-1 text-sm text-slate-400">Live figures calculated from the linked quotes, jobs and invoices saved in JR OS.</p></div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card><div className="flex items-start justify-between"><div><p className="text-sm text-slate-400">Quote conversion rate</p><p className="mt-3 text-3xl font-black">{dashboard.quoteConversionRate.toFixed(1)}%</p></div><span className="rounded-xl bg-violet-500/10 p-2 text-violet-300"><Percent className="size-5" /></span></div><p className="mt-3 text-xs text-slate-500">{dashboard.acceptedQuotes.length} accepted from {dashboard.decidedQuotes.length} decided quote{dashboard.decidedQuotes.length === 1 ? "" : "s"}</p></Card>
        <Card><div className="flex items-start justify-between"><div><p className="text-sm text-slate-400">Jobs in progress</p><p className="mt-3 text-3xl font-black">{dashboard.jobsInProgress.length}</p></div><span className="rounded-xl bg-amber-500/10 p-2 text-amber-300"><BriefcaseBusiness className="size-5" /></span></div><p className="mt-3 text-xs text-slate-500">{dashboard.scheduledJobs.length} more scheduled</p></Card>
        <Card><div className="flex items-start justify-between"><div><p className="text-sm text-slate-400">Unpaid invoices</p><p className="mt-3 text-3xl font-black">{dashboard.outstandingInvoices.length}</p></div><span className="rounded-xl bg-rose-500/10 p-2 text-rose-300"><PoundSterling className="size-5" /></span></div><p className="mt-3 text-xs text-slate-500">{money.format(dashboard.outstanding)} still to collect</p></Card>
        <Card><div className="flex items-start justify-between"><div><p className="text-sm text-slate-400">Expected profit</p><p className="mt-3 text-3xl font-black text-emerald-300">{money.format(dashboard.expectedProfit)}</p></div><span className="rounded-xl bg-emerald-500/10 p-2 text-emerald-300"><TrendingUp className="size-5" /></span></div><p className="mt-3 text-xs text-slate-500">From {dashboard.expectedProfitRecords} active won record{dashboard.expectedProfitRecords === 1 ? "" : "s"}</p></Card>
      </div>
    </section>
      ...
      <Card>
        ...
        <div className="mt-5 space-y-3">
          {dashboard.urgentInvoices.length ? dashboard.urgentInvoices.map((invoice) => {
            const remaining = Math.max(0, invoiceTotal(invoice) - invoice.amountPaid);
