...
export default function SurveysPage() {
  ...
  return <div className="space-y-6">
    ...
    {filtered.length === 0 ? <Card><div className="grid place-items-center py-10 text-center"><ClipboardCheck className="size-10 text-slate-600" /><h2 className="mt-4 text-lg font-bold">No surveys found</h2><p className="mt-2 max-w-md text-sm text-slate-400">Create your first survey and complete it from your phone while walking around the property.</p></div></Card> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((survey) => {
      ...
      return <Link key={survey.id} href={`/surveys/${survey.id}`}><Card className="h-full transition hover:border-cyan-400/40"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-cyan-400">{survey.number}</p><h2 className="mt-2 text-lg font-bold">{job?.title || `${survey.propertyType} survey`}</h2></div><StatusBadge status={survey.status} /></div><p className="mt-3 text-sm text-slate-400">{customer?.name || "No customer linked"}</p><div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-xl bg-slate-950 p-3"><strong className="block text-lg text-white">{survey.circuits.length}</strong><span className="text-slate-500">Circuits</span></div><div className="rounded-xl bg-slate-950 p-3"><strong className="block text-lg text-white">{survey.defects.length}</strong><span className="text-slate-500">Defects</span></div><div className="rounded-xl bg-slate-950 p-3"><strong className="block text-lg text-white">{survey.healthScore}%</strong><span className="text-slate-500">Health</span></div></div></Card></Link>;
    })}</div>}
  </div>;
}