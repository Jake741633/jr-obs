...
export default function SurveysPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      {!fieldMode ? <Button onClick={createSurvey}><Plus className="mr-2 size-4" />New survey</Button> : null}
    </div>

    {fieldMode ? <Card className="border-cyan-400/30"><div className="grid gap-3 md:grid-cols-[1fr_auto]"><label className="grid gap-2 text-sm font-medium text-slate-300"><span>Assigned job for new survey</span><select value={newSurveyJobId} onChange={(event) => { setNewSurveyJobId(event.target.value); setMessage(""); }} className="min-h-11 rounded-xl border border-slate-700 bg-slate-950 px-3"><option value="">Choose assigned job</option>{jobs.items.map((job) => <option key={job.id} value={job.id}>{job.title}</option>)}</select></label><div className="flex items-end"><Button onClick={createSurvey} disabled={!newSurveyJobId}><Plus className="mr-2 size-4" />New field survey</Button></div></div><p className="mt-3 text-xs text-slate-400">Field surveys must be bound to an assigned job before the first cloud save.</p>{message ? <p className="mt-3 text-sm text-amber-200">{message}</p> : null}</Card> : null}
