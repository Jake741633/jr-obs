...
export default function FieldWorkspacePage() {
  ...
  function updateJobStatus(jobId: string, status: CanonicalJobStatus) {
    ...
    if (!job) return;
    const result = transitionJobStatus({ job, nextStatus: status, now: new Date().toISOString(), timelineId: makeId("timeline"), completedBy: "JR OS Mobile" });
  ...
  function startJob(job: Job) {
    ...
    updateJobStatus(job.id, "First fix");
