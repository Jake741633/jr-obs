...
import type { Customer, Job, JobDocument, JobStatus, JobTimelineEntry, SiteDiaryEntry } from "../../lib/models";

const today = () => new Date().toISOString().slice(0, 10);
const nowTime = () => new Date().toTimeString().slice(0, 5);
const blankDiary = { jobId: "", workDate: today(), startedAt: "", finishedAt: "", breakMinutes: "0", completedBy: "Jake", workCompleted: "", delays: "", customerRequests: "", materialsUsed: "", voiceNotes: "" };
...
export default function FieldWorkspacePage() {
  ...
  const todaysJobs = useMemo(() => jobs.items.filter((job) => job.startDate === today() || job.status === "In progress").toSorted((a, b) => a.startDate.localeCompare(b.startDate)), [jobs.items]);
  ...
  function updateJobStatus(jobId: string, status: JobStatus) {
    jobs.setItems((current) => current.map((job) => job.id === jobId ? { ...job, status, updatedAt: new Date().toISOString() } : job));
  }
  ...
  async function saveCompletionPack() {
    ...
    if (selectedPhoto) {
      ...
      if (dataUrl) documents.setItems((current) => [{ id: makeId("document"), jobId: form.jobId, name: `Site completion photo - ${today()}`, category: "Photo", fileName: selectedPhoto.name, mimeType: selectedPhoto.type, dataUrl, externalUrl: "", notes: signOffNotes.trim(), uploadedBy: "Jake", uploadedAt: now, createdAt: now }, ...current]);
    }
    timeline.setItems((current) => [{ id: makeId("timeline"), jobId: form.jobId, milestone: "Job completed", note: `Site checklist completed. Customer sign-off recorded by ${customerName.trim()}.${signOffNotes.trim() ? ` ${signOffNotes.trim()}` : ""}`, completedBy: "JR OS Mobile", completedAt: now, createdAt: now }, ...current]);
  ...
  const ready = jobs.isReady && customers.isReady && diary.isReady && documents.isReady && timeline.isReady;
  if (!ready) return <Card>Loading field workspace…</Card>;

  return <div className="space-y-6">
    <PageHeader eyebrow="Mobile workspace" title="Today on site" description="Start jobs, capture site records, complete testing and hand over from your phone." />
    <section className="grid gap-4 sm:grid-cols-3"><Card><p className="text-sm text-slate-400">Today&apos;s jobs</p><p className="mt-2 text-3xl font-bold">{todaysJobs.length}</p></Card><Card><p className="text-sm text-slate-400">In progress</p><p className="mt-2 text-3xl font-bold">{jobs.items.filter((job) => job.status === "In progress").length}</p></Card><Card><p className="text-sm text-slate-400">Diary records today</p><p className="mt-2 text-3xl font-bold">{todaysEntries.length}</p></Card></section>
    ...
    <MobileTestingProgress activeJobId={form.jobId || todaysJobs.find((job) => job.status === "In progress")?.id} />
