...
import { fieldJobAssignedToOperator, fieldOperatorMemberId, fieldOperatorName } from "../../lib/siteDiaryIdentity-core.mjs";
...
export default function FieldWorkspacePage() {
  ...
  const operatorMemberId = useMemo(() => fieldOperatorMemberId({ identity: identityState.identity, teamMembers: team.items, mode: identityState.mode }), [identityState.identity, identityState.mode, team.items]);
  const cloudFieldMode = identityState.mode !== "local" && identityState.identity?.role === "electrician";
  const selectableDiaryJobs = useMemo(() => jobs.items.filter((job) => (
    !isJobInactiveStatus(job.status)
    && (!cloudFieldMode || fieldJobAssignedToOperator({ job, operatorMemberId }))
  )), [cloudFieldMode, jobs.items, operatorMemberId]);
  ...
  const activeJobAssigned = !cloudFieldMode || fieldJobAssignedToOperator({ job: activeJob, operatorMemberId });
  ...
  function jobStatusSyncBlocked(jobId: string) {
    ...
    if (cloudFieldMode && !fieldJobAssignedToOperator({ job, operatorMemberId })) return true;
  ...
  function updateJobStatus(jobId: string, status: CanonicalJobStatus) {
    const job = jobs.items.find((item) => item.id === jobId);
    if (!job) return false;
    if (cloudFieldMode && !fieldJobAssignedToOperator({ job, operatorMemberId })) {
      setMessage(fieldJobAssignmentMessage);
      return false;
    }
    const syncState = jobStatusSyncState(jobId);
  ...
  function startJob(job: Job) {
    if (cloudFieldMode && (!operatorMemberId || !operatorName)) return setMessage("Your active team identity could not be resolved. Refresh your account before starting work.");
    if (cloudFieldMode && !fieldJobAssignedToOperator({ job, operatorMemberId })) return setMessage(fieldJobAssignmentMessage);
  ...
  function saveDiary(event: FormEvent) {
    ...
    if (!selectedJob || isJobInactiveStatus(selectedJob.status)) return setMessage("The selected active job is no longer available. Refresh the field workspace before saving the site record.");
    if (!operatorName || (cloudFieldMode && !operatorMemberId)) return setMessage("Your active team identity could not be resolved. Refresh your account before saving the site record.");
