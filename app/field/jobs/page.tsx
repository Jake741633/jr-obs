...
export default function MobileJobControlPage() {
  ...
  const pricing = usePricingDocumentsCollection();
  const purchaseLists = usePurchaseListsCollection();
  const rams = useRamsCollection();
  ...
  const ready = [jobs, customers, planner, pricing, purchaseLists, rams, testing, certificates, documents].every((collection) => collection.isReady);
  ...
  const summaries = work.map((job) => {
    ...
    const acceptedPricing = pricing.items.find((document) => document.jobId === job.id && document.status === "Accepted");
    const jobPurchaseLists = purchaseLists.items.filter((list) => list.jobId === job.id);
    const jobRams = rams.items.filter((record) => record.jobId === job.id);
    const jobTesting = testing.items.filter((record) => record.jobId === job.id);
    const jobCertificates = certificates.items.filter((record) => record.jobId === job.id);
    ...
    const readiness = buildMobileJobReadiness({
      ...
      hasAcceptedPricing: Boolean(acceptedPricing || job.sourceQuoteId),
      hasMaterials: jobPurchaseLists.length > 0,
      hasRams: jobRams.length > 0,
      hasTesting: jobTesting.length > 0,
      customerHref: customer ? `/customers/${customer.id}` : "/customers",
      pricingHref: acceptedPricing ? `/quotes/${acceptedPricing.id}` : "/quotes",
    });
    return { job, customer, readiness, jobTesting, jobCertificates, jobDocuments };
  });
  ...
  return <div className="space-y-5 pb-24 sm:space-y-6 sm:pb-0">
    ...
    <section className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      ...
      <Card className="col-span-2 sm:col-span-1"><p className="text-xs text-slate-400">Fully ready</p><p className="mt-2 text-3xl font-bold text-emerald-300">{readyJobs}</p></Card>
    </section>

    {!summaries.length ? <Card><h2 className="font-semibold">No active jobs</h2><p className="mt-2 text-sm text-slate-400">Create or schedule a job and it will appear here.</p></Card> : <section className="space-y-4">{summaries.map(({ job, customer, readiness, jobTesting, jobCertificates, jobDocuments }) => <Card key={job.id} className={job.startDate === today() ? "border-cyan-400/40" : undefined}>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs text-slate-400">
        ...
        <div className="rounded-lg border border-slate-800 p-2"><p className="font-bold text-slate-100">{jobCertificates.length}</p><p>Certificates</p></div>
