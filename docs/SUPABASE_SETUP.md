...
The [Supabase deployment audit, updated 19 September](./security/supabase-deployment-audit-2026-09-11.md)
...
- `electrician`: field, assigned-job, planner and materials records, plus account-scoped device-local testing drafts; complete cloud testing records and canonical certificate authoring and issue remain office-controlled
...
- electricians cannot query or retain canonical electrical testing rows; complete installation test results and office handoff detail remain office-only, while field testing uses account-scoped device-local drafts
...
- electrician typed inventory reads remove material trade and sell prices, price-check metadata, stock unit costs and purchase-list item costs; purchase lists are limited to active jobs assigned to the field identity; inventory edits preserve hidden office pricing; canonical stock-movement history, including job links, movement notes and timestamps, remains office-only; shared stock-location projections expose only the stable ID and display name, keeping vehicle links, office notes and future fields out of field caches
...
- authenticated customer certificate PDF URLs are memory-only and never replayed from browser storage, offline fallback or account backups; each download revalidates the current customer and session, fetches one exact still-issued certificate with a no-store request, and accepts only a credential-free HTTPS URL, while valid office and local/demo links retain their existing behavior
