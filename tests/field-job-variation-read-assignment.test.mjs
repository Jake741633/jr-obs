...
import {
  ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION,
  roleProjectionCacheGeneration,
  roleProjectionCachePolicy,
} from "../lib/cloud/roleProjectionCache-core.mjs";

const migrationName = "20260826101908_scope_field_variation_reads_to_assignments.sql";
...
test("electrician variation reads resolve through an active canonical job assignment", () => {
  const helper = section(
    "create or replace function private.jr_field_variation_targets_assigned_job",
    "revoke execute on function private.jr_field_variation_targets_assigned_job",
  );
  ...
  assert.match(
    migration,
    /revoke execute on function private\.jr_field_variation_targets_assigned_job\(uuid, text, text\)[\s\S]*from public, anon, authenticated, service_role/i,
  );
  assert.match(
    migration,
    /grant execute on function private\.jr_field_variation_targets_assigned_job\(uuid, text, text\)[\s\S]*to authenticated, service_role/i,
  );
});

test("the final field collection policy scopes variations without weakening prior assignment branches", () => {
  ...
  assert.match(
    policy,
    /when 'jr-os-job-variations' then private\.jr_field_variation_targets_assigned_job\(\s*organisation_id,\s*customer_source_id,\s*job_source_id\s*\)/i,
  );
...
test("stale tenant-wide electrician variation caches purge before offline fallback", () => {
  assert.equal(ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION, "20260826101908");
  assert.equal(
    roleProjectionCacheGeneration({ storageKey: "jr-os-job-variations", role: "electrician" }),
    ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION,
  );
  assert.equal(
    roleProjectionCachePolicy({
      storageKey: "jr-os-job-variations",
      role: "electrician",
      mode: "cloud",
      generation: "20260809",
    }),
  ...
  assert.equal(
    roleProjectionCachePolicy({
      ...
      role: "electrician",
      mode: "cloud",
      generation: ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION,
    }),
    "keep",
  );
  assert.match(cache, /ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION = "20260826101908"/);
  assert.match(
    cache,
    /role === "electrician" && storageKey === "jr-os-job-variations"[\s\S]*return ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION/,
  );
  assert.match(cache, /expectedGeneration && generation !== expectedGeneration[\s\S]*return "purge"/);
  assert.match(cacheTypes, /ELECTRICIAN_JOB_VARIATION_CACHE_GENERATION: "20260826101908"/);
...
test("variation writes remain default-denied for electricians", () => {
  assert.deepEqual(
...
test("live RLS coverage retains assigned variations and rejects wider field reads", () => {
  for (const phrase of [
...
test("recovery, guidance and deployment marker retain variation assignment scoping", () => {
  const diaryProgress = recovery.indexOf("20260820170000_preserve_field_site_diary_progress.sql");
  const variationScope = recovery.indexOf(migrationName);
  assert.ok(diaryProgress >= 0 && variationScope > diaryProgress);
  assert.match(recovery.slice(variationScope - 120, variationScope + migrationName.length + 50), /begin;[\s\S]*\\ir[\s\S]*commit;/i);
  assert.match(setup, /job variations[\s\S]*limited to assigned jobs/i);
