...
const fieldPurchaseListReadCoverage = [
  '    const assignedPurchaseList = source("field-purchase-list-assigned-a");',
  '    const unassignedPurchaseList = source("field-purchase-list-unassigned-a");',
  '    const unboundPurchaseList = source("field-purchase-list-unbound-a");',
  '    const crossTenantPurchaseList = source("field-purchase-list-cross-tenant-b");',
  '    const revokedPurchaseJob = source("field-purchase-list-revoked-job-a");',
  '    const revokedJobPurchaseList = source("field-purchase-list-revoked-a");',
  '    const deletedPurchaseJob = source("field-purchase-list-deleted-job-a");',
  '    const deletedJobPurchaseList = source("field-purchase-list-deleted-a");',
  '    const purchaseListPayload = { number: "PO-SEC-READ", title: "Private supplier order", items: [{ id: source("field-purchase-item-read"), description: "Containment", supplier: "Private Supplier", stockCode: "PRIVATE-42", supplierUrl: "https://supplier.example/private-order", quantity: 24, unitCost: 8.75, status: "Needed" }], pricingDocumentId: source("field-purchase-pricing-document"), notes: "Private delivery and access note", createdAt: "2026-09-03T14:10:00.000Z", updatedAt: "2026-09-03T14:10:00.000Z" };',
  '    await expectAllowed(await insertRecord(accounts.A.office, "purchase_lists", typedRecord(organisationA, assignedPurchaseList, null, jobA, { ...purchaseListPayload, id: assignedPurchaseList, jobId: jobA, title: "Assigned private supplier order" })), "Office should create a production-shaped assigned purchase-list fixture");',
  '    await expectAllowed(await insertRecord(accounts.A.office, "purchase_lists", typedRecord(organisationA, unassignedPurchaseList, null, otherCustomerJobA, { ...purchaseListPayload, id: unassignedPurchaseList, jobId: otherCustomerJobA, title: "Unassigned private supplier order" })), "Office should create an unassigned purchase-list fixture");',
  '    await expectAllowed(await insertRecord(accounts.A.office, "purchase_lists", typedRecord(organisationA, unboundPurchaseList, null, null, { ...purchaseListPayload, id: unboundPurchaseList, jobId: undefined, title: "Unbound private supplier order" })), "Office should create an unbound purchase-list fixture");',
  '    await expectAllowed(await insertRecord(accounts.B.office, "purchase_lists", typedRecord(organisationB, crossTenantPurchaseList, null, crossTenantBuilderJobB, { ...purchaseListPayload, id: crossTenantPurchaseList, jobId: crossTenantBuilderJobB, title: "Other tenant assigned supplier order" })), "Another organisation should create its assigned purchase-list fixture");',
  '    await expectAllowed(await insertRecord(accounts.A.office, "jobs", typedRecord(organisationA, revokedPurchaseJob, customerA, null, { title: "Revoked purchase-list job", status: "First fix", assignedTo: [fieldTeamA, fieldTeamCoworkerA] })), "Office should create an assigned job for purchase-list revocation coverage");',
  '    await expectAllowed(await insertRecord(accounts.A.office, "purchase_lists", typedRecord(organisationA, revokedJobPurchaseList, null, revokedPurchaseJob, { ...purchaseListPayload, id: revokedJobPurchaseList, jobId: revokedPurchaseJob, title: "Purchase list for revoked job" })), "Office should create a null-customer purchase list before assignment revocation");',
  '    await expectAllowed(await insertRecord(accounts.A.office, "jobs", typedRecord(organisationA, deletedPurchaseJob, customerA, null, { title: "Deleted purchase-list job", status: "First fix", assignedTo: [fieldTeamA, fieldTeamCoworkerA] })), "Office should create an assigned job for purchase-list deletion coverage");',
  '    await expectAllowed(await insertRecord(accounts.A.office, "purchase_lists", typedRecord(organisationA, deletedJobPurchaseList, null, deletedPurchaseJob, { ...purchaseListPayload, id: deletedJobPurchaseList, jobId: deletedPurchaseJob, title: "Purchase list for deleted job" })), "Office should create a null-customer purchase list before job deletion");',
  '',
  '    const assignedFieldPurchaseList = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id,customer_source_id,job_source_id,payload&source_id=eq." + assignedPurchaseList);',
  '    await expectAllowed(assignedFieldPurchaseList, "Assigned electrician purchase-list query should execute");',
  '    assert.equal(assignedFieldPurchaseList.payload.length, 1, "Assigned electrician should retain the production-shaped assigned purchase list");',
  '    assert.equal(assignedFieldPurchaseList.payload[0].customer_source_id, null, "Production purchase-list envelopes must retain their null customer binding");',
  '    assert.equal(assignedFieldPurchaseList.payload[0].job_source_id, jobA, "Field purchase lists must retain their canonical assigned job binding");',
  '    assert.equal(assignedFieldPurchaseList.payload[0].payload.items[0].supplier, "Private Supplier", "Assigned field purchase lists should retain operational supplier data");',
  '    assert.equal(assignedFieldPurchaseList.payload[0].payload.items[0].unitCost, undefined, "Field purchase-list projection must omit item unit costs");',
  '    assert.equal(assignedFieldPurchaseList.payload[0].payload.pricingDocumentId, undefined, "Field purchase-list projection must omit pricing-document linkage");',
  '    const officeAssignedPurchaseList = await listRecords(accounts.A.office, "purchase_lists", "select=source_id,customer_source_id,job_source_id,payload&source_id=eq." + assignedPurchaseList);',
  '    await expectAllowed(officeAssignedPurchaseList, "Office assigned purchase-list query should execute");',
  '    assert.equal(officeAssignedPurchaseList.payload.length, 1, "Office should retain the complete assigned purchase list");',
  '    assert.equal(officeAssignedPurchaseList.payload[0].customer_source_id, null);',
  '    assert.equal(officeAssignedPurchaseList.payload[0].job_source_id, jobA);',
  '    assert.equal(officeAssignedPurchaseList.payload[0].payload.items[0].unitCost, 8.75, "Office should retain assigned purchase-list costs");',
  '    assert.equal(officeAssignedPurchaseList.payload[0].payload.pricingDocumentId, source("field-purchase-pricing-document"), "Office should retain assigned purchase-list pricing linkage");',
  '    const coworkerFieldPurchaseList = await listRecords(accounts.A.coworker, "field_purchase_lists", "select=source_id&source_id=eq." + assignedPurchaseList);',
  '    await expectAllowed(coworkerFieldPurchaseList, "Co-assigned electrician purchase-list query should execute");',
  '    assert.equal(coworkerFieldPurchaseList.payload.length, 1, "Co-assigned electrician should retain the assigned purchase list");',
  '    const electricianCanonicalPurchaseList = await listRecords(accounts.A.electrician, "purchase_lists", "select=source_id&source_id=eq." + assignedPurchaseList);',
  '    await expectAllowed(electricianCanonicalPurchaseList, "Electrician canonical purchase-list query should fail closed");',
  '    assert.deepEqual(electricianCanonicalPurchaseList.payload, [], "Electrician must not read canonical purchase lists");',
  '    const customerFieldPurchaseList = await listRecords(accounts.A.customer, "field_purchase_lists", "select=source_id&source_id=eq." + assignedPurchaseList);',
  '    await expectAllowed(customerFieldPurchaseList, "Customer field purchase-list query should execute safely");',
  '    assert.deepEqual(customerFieldPurchaseList.payload, [], "Customers must not read field purchase lists");',
  '    const unassignedFieldPurchaseList = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + unassignedPurchaseList);',
  '    await expectAllowed(unassignedFieldPurchaseList, "Unassigned purchase-list query should execute safely");',
  '    assert.deepEqual(unassignedFieldPurchaseList.payload, [], "Electrician must not read an unassigned same-tenant purchase list");',
  '    const unboundFieldPurchaseList = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + unboundPurchaseList);',
  '    await expectAllowed(unboundFieldPurchaseList, "Unbound purchase-list query should execute safely");',
  '    assert.deepEqual(unboundFieldPurchaseList.payload, [], "Electrician must not read a purchase list without a canonical job");',
  '    const crossTenantFieldPurchaseList = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + crossTenantPurchaseList);',
  '    await expectAllowed(crossTenantFieldPurchaseList, "Cross-tenant purchase-list query should execute safely");',
  '    assert.deepEqual(crossTenantFieldPurchaseList.payload, [], "Assigned electrician must not read another organisation\'s purchase list");',
  '    const tenantBAssignedPurchaseList = await listRecords(accounts.B.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + crossTenantPurchaseList);',
  '    await expectAllowed(tenantBAssignedPurchaseList, "Tenant B assigned purchase-list query should execute");',
  '    assert.equal(tenantBAssignedPurchaseList.payload.length, 1, "Tenant B assigned electrician should retain its own purchase list");',
  '    const purchaseListUnboundUser = await createUser("a-electrician-purchase-list-unbound");',
  '    context.users.push(purchaseListUnboundUser);',
  '    await createProfile(purchaseListUnboundUser, organisationA, "electrician");',
  '    const purchaseListUnboundAccount = { ...purchaseListUnboundUser, ...(await signIn(purchaseListUnboundUser)), organisationId: organisationA };',
  '    const noIdentityPurchaseList = await listRecords(purchaseListUnboundAccount, "field_purchase_lists", "select=source_id&source_id=eq." + assignedPurchaseList);',
  '    await expectAllowed(noIdentityPurchaseList, "Unbound field identity purchase-list query should execute safely");',
  '    assert.deepEqual(noIdentityPurchaseList.payload, [], "Electrician without an active field identity must not read purchase lists");',
  '    const officeUnassignedPurchaseList = await listRecords(accounts.A.office, "purchase_lists", "select=source_id,payload&source_id=eq." + unassignedPurchaseList);',
  '    await expectAllowed(officeUnassignedPurchaseList, "Office unassigned purchase-list query should execute");',
  '    assert.equal(officeUnassignedPurchaseList.payload.length, 1, "Office should retain unassigned purchase-list access");',
  '    assert.equal(officeUnassignedPurchaseList.payload[0].payload.items[0].unitCost, 8.75, "Office should retain canonical purchase-list costs");',
  '    const fieldPurchaseListBeforeRevocation = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + revokedJobPurchaseList);',
  '    await expectAllowed(fieldPurchaseListBeforeRevocation, "Assigned purchase-list query before assignment revocation should execute");',
  '    assert.equal(fieldPurchaseListBeforeRevocation.payload.length, 1, "Electrician should read a purchase list while the job remains assigned");',
  '    const revokedPurchaseJobRecord = await listRecords(accounts.A.office, "jobs", "select=payload&source_id=eq." + revokedPurchaseJob);',
  '    await expectAllowed(revokedPurchaseJobRecord, "Office revocation-job query should execute");',
  '    assert.equal(revokedPurchaseJobRecord.payload.length, 1);',
  '    await expectAllowed(await patchRecords(accounts.A.owner, "jobs", "source_id=eq." + revokedPurchaseJob, { payload: { ...revokedPurchaseJobRecord.payload[0].payload, assignedTo: [fieldTeamCoworkerA] } }), "Owner should selectively revoke the primary purchase-list job assignment");',
  '    const fieldPurchaseListAfterRevocation = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + revokedJobPurchaseList);',
  '    await expectAllowed(fieldPurchaseListAfterRevocation, "Revoked purchase-list query should execute safely");',
  '    assert.deepEqual(fieldPurchaseListAfterRevocation.payload, [], "Electrician must immediately lose purchase-list reads after assignment revocation");',
  '    const coworkerPurchaseListAfterRevocation = await listRecords(accounts.A.coworker, "field_purchase_lists", "select=source_id&source_id=eq." + revokedJobPurchaseList);',
  '    await expectAllowed(coworkerPurchaseListAfterRevocation, "Remaining-assignee purchase-list query should execute");',
  '    assert.equal(coworkerPurchaseListAfterRevocation.payload.length, 1, "Co-assigned electrician should retain purchase-list access after another assignment is revoked");',
  '    const officePurchaseListAfterRevocation = await listRecords(accounts.A.office, "purchase_lists", "select=source_id,payload&source_id=eq." + revokedJobPurchaseList);',
  '    await expectAllowed(officePurchaseListAfterRevocation, "Office revoked purchase-list query should execute");',
  '    assert.equal(officePurchaseListAfterRevocation.payload.length, 1, "Office should retain canonical purchase-list history after assignment revocation");',
  '    assert.equal(officePurchaseListAfterRevocation.payload[0].payload.items[0].unitCost, 8.75);',
  '    const fieldPurchaseListBeforeJobDelete = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + deletedJobPurchaseList);',
  '    await expectAllowed(fieldPurchaseListBeforeJobDelete, "Assigned purchase-list query before job deletion should execute");',
  '    assert.equal(fieldPurchaseListBeforeJobDelete.payload.length, 1, "Electrician should read a purchase list while the job is active and assigned");',
  '    await expectAllowed(await patchRecords(accounts.A.owner, "jobs", "source_id=eq." + deletedPurchaseJob, { deleted_at: new Date().toISOString() }), "Owner should soft-delete the assigned purchase-list job");',
  '    const fieldPurchaseListAfterJobDelete = await listRecords(accounts.A.electrician, "field_purchase_lists", "select=source_id&source_id=eq." + deletedJobPurchaseList);',
  '    await expectAllowed(fieldPurchaseListAfterJobDelete, "Deleted-job purchase-list query should execute safely");',
  '    assert.deepEqual(fieldPurchaseListAfterJobDelete.payload, [], "Electrician must not read a purchase list for a soft-deleted job");',
  '    const officePurchaseListAfterJobDelete = await listRecords(accounts.A.office, "purchase_lists", "select=source_id,payload&source_id=eq." + deletedJobPurchaseList);',
  '    await expectAllowed(officePurchaseListAfterJobDelete, "Office deleted-job purchase-list query should execute");',
  '    assert.equal(officePurchaseListAfterJobDelete.payload.length, 1, "Office should retain canonical purchase-list history after job deletion");',
  '    assert.equal(officePurchaseListAfterJobDelete.payload[0].payload.notes, "Private delivery and access note");',
  '',
].join("\n");

const fieldCertificateDefaultsOfficeCoverage = [
  '    const certificateDefaultsA = source("field-certificate-defaults-a");',
  '    const certificateDefaultsB = source("field-certificate-defaults-b");',
  '    const certificateDefaultsPayload = { inspectorName: "Private Inspector", schemeProvider: "Private Scheme Provider", registrationNumber: "PRIVATE-REG-4242", defaultType: "Electrical Installation Certificate", defaultOutcome: "Satisfactory", certificatePrefix: "PRIVATE-CERT", nextInspectionYears: 5, notes: "Private office certification note", updatedAt: "2026-09-03T14:40:00.000Z" };',
  '    await expectAllowed(await insertRecord(accounts.A.office, "cloud_collections", genericRecord(organisationA, "jr-os-certificate-defaults", certificateDefaultsA, accounts.A.office, undefined, undefined, { ...certificateDefaultsPayload, id: certificateDefaultsA })), "Office should create complete certificate defaults");',
  '    await expectAllowed(await insertRecord(accounts.B.office, "cloud_collections", genericRecord(organisationB, "jr-os-certificate-defaults", certificateDefaultsB, accounts.B.office, undefined, undefined, { ...certificateDefaultsPayload, id: certificateDefaultsB, inspectorName: "Tenant B Private Inspector", registrationNumber: "PRIVATE-REG-B" })), "Tenant B office should create complete certificate defaults");',
  '    const officeCertificateDefaults = await listRecords(accounts.A.office, "cloud_collections", "select=*&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(officeCertificateDefaults, "Office certificate-default query should execute");',
  '    assert.equal(officeCertificateDefaults.payload.length, 1, "Office should retain complete certificate defaults");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.inspectorName, "Private Inspector");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.schemeProvider, "Private Scheme Provider");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.registrationNumber, "PRIVATE-REG-4242");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.defaultType, "Electrical Installation Certificate");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.defaultOutcome, "Satisfactory");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.certificatePrefix, "PRIVATE-CERT");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.nextInspectionYears, 5);',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.notes, "Private office certification note");',
  '    assert.equal(officeCertificateDefaults.payload[0].payload.updatedAt, "2026-09-03T14:40:00.000Z");',
  '    const tenantBOfficeCertificateDefaults = await listRecords(accounts.B.office, "cloud_collections", "select=*&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsB);',
  '    await expectAllowed(tenantBOfficeCertificateDefaults, "Tenant B office certificate-default query should execute");',
  '    assert.equal(tenantBOfficeCertificateDefaults.payload.length, 1, "Tenant B office should retain its complete certificate defaults");',
  '    const freshCertificateProjection = await service("/rest/v1/field_cloud_collections?select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(freshCertificateProjection, "Service certificate-default projection query should execute");',
  '    assert.deepEqual(freshCertificateProjection.payload, [], "New certificate defaults must not materialize in the field projection");',
  '    const staleCertificateProjectionA = await service("/rest/v1/field_cloud_collections", { method: "POST", body: officeCertificateDefaults.payload[0], extraHeaders: { Prefer: "return=representation" } });',
  '    await expectAllowed(staleCertificateProjectionA, "Service should seed a pre-migration certificate-default projection");',
  '    assert.equal(staleCertificateProjectionA.payload.length, 1, "The stale certificate-default projection fixture must exist outside RLS");',
  '    const staleCertificateProjectionB = await service("/rest/v1/field_cloud_collections", { method: "POST", body: tenantBOfficeCertificateDefaults.payload[0], extraHeaders: { Prefer: "return=representation" } });',
  '    await expectAllowed(staleCertificateProjectionB, "Service should seed another-tenant pre-migration certificate-default projection");',
  '    assert.equal(staleCertificateProjectionB.payload.length, 1);',
  '',
  '    const assignedFieldCertificateDefaults = await listRecords(accounts.A.electrician, "field_cloud_collections", "select=source_id,payload&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(assignedFieldCertificateDefaults, "Assigned electrician certificate-default projection query should execute safely");',
  '    assert.deepEqual(assignedFieldCertificateDefaults.payload, [], "Assigned electrician must not read certificate defaults from the field projection");',
  '    const coworkerFieldCertificateDefaults = await listRecords(accounts.A.coworker, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(coworkerFieldCertificateDefaults, "Co-assigned electrician certificate-default projection query should execute safely");',
  '    assert.deepEqual(coworkerFieldCertificateDefaults.payload, [], "Co-assigned electrician must not read certificate defaults from the field projection");',
  '    const certificateDefaultsUnboundUser = await createUser("a-electrician-certificate-defaults-unbound");',
  '    context.users.push(certificateDefaultsUnboundUser);',
  '    await createProfile(certificateDefaultsUnboundUser, organisationA, "electrician");',
  '    const certificateDefaultsUnboundAccount = { ...certificateDefaultsUnboundUser, ...(await signIn(certificateDefaultsUnboundUser)), organisationId: organisationA };',
  '    const noIdentityCertificateDefaults = await listRecords(certificateDefaultsUnboundAccount, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(noIdentityCertificateDefaults, "Unbound field identity certificate-default query should execute safely");',
  '    assert.deepEqual(noIdentityCertificateDefaults.payload, [], "Electrician without an active field identity must not read certificate defaults");',
  '    const tenantBFieldCertificateDefaults = await listRecords(accounts.B.electrician, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsB);',
  '    await expectAllowed(tenantBFieldCertificateDefaults, "Tenant B certificate-default projection query should execute safely");',
  '    assert.deepEqual(tenantBFieldCertificateDefaults.payload, [], "Tenant B electrician must not read its own organisation certificate defaults");',
  '    const customerFieldCertificateDefaults = await listRecords(accounts.A.customer, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(customerFieldCertificateDefaults, "Customer certificate-default projection query should execute safely");',
  '    assert.deepEqual(customerFieldCertificateDefaults.payload, [], "Customers must not read certificate defaults from the field projection");',
  '    const electricianCanonicalCertificateDefaults = await listRecords(accounts.A.electrician, "cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(electricianCanonicalCertificateDefaults, "Electrician canonical certificate-default query should fail closed");',
  '    assert.deepEqual(electricianCanonicalCertificateDefaults.payload, [], "Electrician must not read canonical certificate defaults");',
  '    const customerCanonicalCertificateDefaults = await listRecords(accounts.A.customer, "cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(customerCanonicalCertificateDefaults, "Customer canonical certificate-default query should fail closed");',
  '    assert.deepEqual(customerCanonicalCertificateDefaults.payload, [], "Customers must not read canonical certificate defaults");',
  '    const crossTenantCanonicalCertificateDefaults = await listRecords(accounts.B.office, "cloud_collections", "select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(crossTenantCanonicalCertificateDefaults, "Cross-tenant canonical certificate-default query should execute safely");',
  '    assert.deepEqual(crossTenantCanonicalCertificateDefaults.payload, [], "Another organisation must not read canonical certificate defaults");',
  '    await expectDenied(await insertRecord(accounts.A.electrician, "cloud_collections", genericRecord(organisationA, "jr-os-certificate-defaults", source("field-certificate-defaults-denied-write"), accounts.A.electrician, undefined, undefined, certificateDefaultsPayload)), "Electrician direct certificate-default writes must fail closed");',
  '    const deniedCertificateDefaultsRpcId = source("field-certificate-defaults-denied-rpc");',
  '    await expectDeniedWithCode(await authenticated(accounts.A.electrician, "/rest/v1/rpc/jr_field_save_collection", { method: "POST", body: { collection_key_value: "jr-os-certificate-defaults", record_source_id: deniedCertificateDefaultsRpcId, expected_version: 0, record_payload: { ...certificateDefaultsPayload, id: deniedCertificateDefaultsRpcId }, mutation_id: crypto.randomUUID() } }), "42501", "Field collection RPC must reject certificate-default writes");',
  '    await expectAllowed(await patchRecords(accounts.A.owner, "cloud_collections", "collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA, { deleted_at: new Date().toISOString() }), "Owner should tombstone the certificate-default fixture");',
  '    const officeDeletedCertificateDefaults = await listRecords(accounts.A.office, "cloud_collections", "select=source_id,payload,deleted_at&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(officeDeletedCertificateDefaults, "Office deleted certificate-default query should execute");',
  '    assert.equal(officeDeletedCertificateDefaults.payload.length, 1, "Office should retain certificate-default tombstone history");',
  '    assert.ok(officeDeletedCertificateDefaults.payload[0].deleted_at);',
  '    assert.equal(officeDeletedCertificateDefaults.payload[0].payload.notes, "Private office certification note");',
  '    const staleCertificateProjectionAfterDelete = await service("/rest/v1/field_cloud_collections?select=source_id&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(staleCertificateProjectionAfterDelete, "Service deleted certificate-default projection query should execute");',
  '    assert.deepEqual(staleCertificateProjectionAfterDelete.payload, [], "Tombstoning certificate defaults must remove the stale field projection");',
  '    const fieldDeletedCertificateDefaults = await listRecords(accounts.A.electrician, "field_cloud_collections", "select=source_id,deleted_at&collection_key=eq.jr-os-certificate-defaults&source_id=eq." + certificateDefaultsA);',
  '    await expectAllowed(fieldDeletedCertificateDefaults, "Electrician deleted certificate-default query should fail closed");',
  '    assert.deepEqual(fieldDeletedCertificateDefaults.payload, [], "Electrician must not read deleted certificate defaults");',
  '',
].join("\n");

const fieldFleetOfficeCoverage = [
  '    const fleetA = source("field-fleet-a");',
  '    const fleetB = source("field-fleet-b");',
  '    const fleetPayload = { registration: "PRIVATE-REG-A", make: "Ford", model: "Transit", status: "Active", assignedTeamMemberId: fieldTeamA, motDue: "2027-01-15", insuranceDue: "2027-02-16", serviceDue: "2027-03-17", currentMileage: 42424, notes: "Private office vehicle note", createdAt: "2026-09-03T15:30:00.000Z", updatedAt: "2026-09-03T15:30:00.000Z" };',
  '    await expectAllowed(await insertRecord(accounts.A.office, "cloud_collections", genericRecord(organisationA, "jr-os-fleet", fleetA, accounts.A.office, undefined, undefined, { ...fleetPayload, id: fleetA })), "Office should create a complete fleet record");',
  '    await expectAllowed(await insertRecord(accounts.B.office, "cloud_collections", genericRecord(organisationB, "jr-os-fleet", fleetB, accounts.B.office, undefined, undefined, { ...fleetPayload, id: fleetB, registration: "PRIVATE-REG-B", assignedTeamMemberId: null, notes: "Tenant B private fleet note" })), "Tenant B office should create a complete fleet record");',
  '    const officeFleet = await listRecords(accounts.A.office, "cloud_collections", "select=*&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(officeFleet, "Office fleet query should execute");',
  '    assert.equal(officeFleet.payload.length, 1, "Office should retain complete fleet records");',
  '    assert.equal(officeFleet.payload[0].payload.registration, "PRIVATE-REG-A");',
  '    assert.equal(officeFleet.payload[0].payload.assignedTeamMemberId, fieldTeamA);',
  '    assert.equal(officeFleet.payload[0].payload.motDue, "2027-01-15");',
  '    assert.equal(officeFleet.payload[0].payload.insuranceDue, "2027-02-16");',
  '    assert.equal(officeFleet.payload[0].payload.serviceDue, "2027-03-17");',
  '    assert.equal(officeFleet.payload[0].payload.currentMileage, 42424);',
  '    assert.equal(officeFleet.payload[0].payload.notes, "Private office vehicle note");',
  '    const tenantBOfficeFleet = await listRecords(accounts.B.office, "cloud_collections", "select=*&collection_key=eq.jr-os-fleet&source_id=eq." + fleetB);',
  '    await expectAllowed(tenantBOfficeFleet, "Tenant B office fleet query should execute");',
  '    assert.equal(tenantBOfficeFleet.payload.length, 1, "Tenant B office should retain its complete fleet record");',
  '    const freshFleetProjection = await service("/rest/v1/field_cloud_collections?select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(freshFleetProjection, "Service fresh fleet projection query should execute");',
  '    assert.deepEqual(freshFleetProjection.payload, [], "New fleet records must not materialize in the field projection");',
  '    const staleFleetProjectionA = await service("/rest/v1/field_cloud_collections", { method: "POST", body: officeFleet.payload[0], extraHeaders: { Prefer: "return=representation" } });',
  '    await expectAllowed(staleFleetProjectionA, "Service should seed a pre-migration fleet projection");',
  '    assert.equal(staleFleetProjectionA.payload.length, 1, "The stale fleet projection fixture must exist outside RLS");',
  '    const staleFleetProjectionB = await service("/rest/v1/field_cloud_collections", { method: "POST", body: tenantBOfficeFleet.payload[0], extraHeaders: { Prefer: "return=representation" } });',
  '    await expectAllowed(staleFleetProjectionB, "Service should seed another-tenant pre-migration fleet projection");',
  '    assert.equal(staleFleetProjectionB.payload.length, 1);',
  '',
  '    const assignedFieldFleet = await listRecords(accounts.A.electrician, "field_cloud_collections", "select=source_id,payload&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(assignedFieldFleet, "Assigned electrician fleet projection query should execute safely");',
  '    assert.deepEqual(assignedFieldFleet.payload, [], "Assigned electrician must not read fleet records from the field projection");',
  '    const coworkerFieldFleet = await listRecords(accounts.A.coworker, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(coworkerFieldFleet, "Co-assigned electrician fleet projection query should execute safely");',
  '    assert.deepEqual(coworkerFieldFleet.payload, [], "Co-assigned electrician must not read fleet records from the field projection");',
  '    const fleetUnboundUser = await createUser("a-electrician-fleet-unbound");',
  '    context.users.push(fleetUnboundUser);',
  '    await createProfile(fleetUnboundUser, organisationA, "electrician");',
  '    const fleetUnboundAccount = { ...fleetUnboundUser, ...(await signIn(fleetUnboundUser)), organisationId: organisationA };',
  '    const noIdentityFieldFleet = await listRecords(fleetUnboundAccount, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(noIdentityFieldFleet, "Unbound field identity fleet query should execute safely");',
  '    assert.deepEqual(noIdentityFieldFleet.payload, [], "Electrician without an active field identity must not read fleet records");',
  '    const tenantBFieldFleet = await listRecords(accounts.B.electrician, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetB);',
  '    await expectAllowed(tenantBFieldFleet, "Tenant B fleet projection query should execute safely");',
  '    assert.deepEqual(tenantBFieldFleet.payload, [], "Tenant B electrician must not read its own organisation fleet records");',
  '    const customerFieldFleet = await listRecords(accounts.A.customer, "field_cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(customerFieldFleet, "Customer fleet projection query should execute safely");',
  '    assert.deepEqual(customerFieldFleet.payload, [], "Customers must not read fleet records from the field projection");',
  '    const electricianCanonicalFleet = await listRecords(accounts.A.electrician, "cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(electricianCanonicalFleet, "Electrician canonical fleet query should fail closed");',
  '    assert.deepEqual(electricianCanonicalFleet.payload, [], "Electrician must not read canonical fleet records");',
  '    const customerCanonicalFleet = await listRecords(accounts.A.customer, "cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(customerCanonicalFleet, "Customer canonical fleet query should fail closed");',
  '    assert.deepEqual(customerCanonicalFleet.payload, [], "Customers must not read canonical fleet records");',
  '    const crossTenantCanonicalFleet = await listRecords(accounts.B.office, "cloud_collections", "select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(crossTenantCanonicalFleet, "Cross-tenant canonical fleet query should execute safely");',
  '    assert.deepEqual(crossTenantCanonicalFleet.payload, [], "Another organisation must not read canonical fleet records");',
  '    await expectDenied(await insertRecord(accounts.A.electrician, "cloud_collections", genericRecord(organisationA, "jr-os-fleet", source("field-fleet-denied-write"), accounts.A.electrician, undefined, undefined, fleetPayload)), "Electrician direct fleet writes must fail closed");',
  '    const deniedFleetRpcId = source("field-fleet-denied-rpc");',
  '    await expectDeniedWithCode(await authenticated(accounts.A.electrician, "/rest/v1/rpc/jr_field_save_collection", { method: "POST", body: { collection_key_value: "jr-os-fleet", record_source_id: deniedFleetRpcId, expected_version: 0, record_payload: { ...fleetPayload, id: deniedFleetRpcId }, mutation_id: crypto.randomUUID() } }), "42501", "Field collection RPC must reject fleet writes");',
  '    await expectAllowed(await patchRecords(accounts.A.owner, "cloud_collections", "collection_key=eq.jr-os-fleet&source_id=eq." + fleetA, { deleted_at: new Date().toISOString() }), "Owner should tombstone the fleet fixture");',
  '    const officeDeletedFleet = await listRecords(accounts.A.office, "cloud_collections", "select=source_id,payload,deleted_at&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(officeDeletedFleet, "Office deleted fleet query should execute");',
  '    assert.equal(officeDeletedFleet.payload.length, 1, "Office should retain fleet tombstone history");',
  '    assert.ok(officeDeletedFleet.payload[0].deleted_at);',
  '    assert.equal(officeDeletedFleet.payload[0].payload.notes, "Private office vehicle note");',
  '    const staleFleetProjectionAfterDelete = await service("/rest/v1/field_cloud_collections?select=source_id&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(staleFleetProjectionAfterDelete, "Service deleted fleet projection query should execute");',
  '    assert.deepEqual(staleFleetProjectionAfterDelete.payload, [], "Tombstoning fleet records must remove the stale field projection");',
  '    const fieldDeletedFleet = await listRecords(accounts.A.electrician, "field_cloud_collections", "select=source_id,deleted_at&collection_key=eq.jr-os-fleet&source_id=eq." + fleetA);',
  '    await expectAllowed(fieldDeletedFleet, "Electrician deleted fleet query should fail closed");',
  '    assert.deepEqual(fieldDeletedFleet.payload, [], "Electrician must not read deleted fleet records");',
  '',
].join("\n");
...
const secureFieldCasesSnippet = `    // Direct field writes are closed; only existing planner/timesheet identity-bound policies remain.
    const closedFieldCases = [
      ...
      ["purchase_lists", source("purchase-a"), { number: "PO-SEC-001", title: "Assigned supplier order", items: [{ id: source("purchase-item-a"), description: "Cable", supplier: "Assigned Supplier", stockCode: "CABLE-A", supplierUrl: "https://supplier.example/assigned", quantity: 12, unitCost: 3.5, status: "Ordered" }], notes: "Deliver through the assigned-site entrance", status: "Draft" }],
      ["certificates", source("certificate-a"), { status: "Draft" }],
      ["electrical_testing_records", source("testing-a"), { status: "Draft" }],
      ["job_documents", source("document-a"), { category: "Photo" }],
    ];
...
try {
  const supportedSource = source
    ...
    .replace(genericCasesStart, `${fieldRamsOfficeCoverage}${fieldBuilderReadCoverage}${fieldTimelineCoverage}${fieldTimelineFinanceCoverage}${fieldSiteDiaryCoverage}${fieldVariationCoverage}${fieldProgressReadCoverage}${fieldProgressUpdateEnvelopeCoverage}${fieldMaterialUsageReadCoverage}${fieldJobTaskReadCoverage}${fieldJobQaReadCoverage}${fieldJobCompletionOfficeCoverage}${fieldPurchaseListReadCoverage}${fieldCertificateDefaultsOfficeCoverage}${fieldFleetOfficeCoverage}${fieldMutationCoverage}${genericCasesStart}`)
