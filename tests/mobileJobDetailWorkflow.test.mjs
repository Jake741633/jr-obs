...
test("job detail page preserves flexible wrapping for long records", () => {
  assert.match(jobDetailPage, /flex\s+flex-wrap\s+items-start\s+justify-between\s+gap-4/);
...
test("job detail workflow actions remain touch friendly and can wrap on phones", () => {
...
test("job detail cards keep responsive single-column phone layouts", () => {
  assert.match(jobDetailPage, /grid\s+gap-4\s+sm:grid-cols-3/);
  assert.match(jobDetailPage, /grid\s+gap-4\s+md:grid-cols-2/);
  assert.match(jobDetailPage, /flex\s+flex-col\s+gap-4\s+md:flex-row\s+md:items-center\s+md:justify-between/);
});
