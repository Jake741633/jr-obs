...
test("automatic reconnect sync remains bound to the active organisation and user", () => {
  ...
  assert.match(repository, /const userId = activeUserId\(\);/);
...
test("in-flight background replay aborts when identity ownership changes", () => {
  const ownershipChecks = repository.match(/activeOrganisationId\(\) !== organisationId \|\| activeUserId\(\) !== userId/g) ?? [];
  assert.ok(ownershipChecks.length >= 2, "background replay must recheck organisation and user ownership before and after remote reads");
