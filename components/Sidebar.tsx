...
import { activeSidebarHref } from "../lib/sidebarNavigationActive-core.mjs";
...
export default function Sidebar() {
  ...
  const secondary = secondaryNavigation.filter(([, href]) => (
    permitted(href)
    && href !== "/cloud"
    && !(identity?.role === "electrician" && href === "/field/jobs")
  ));
  const activeHref = activeSidebarHref({
    pathname,
    hrefs: [
      ...primary.map(({ href }) => href),
      ...secondary.map(([, href]) => href),
    ],
    isElectrician: identity?.role === "electrician",
  });
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 flex-col border-r border-slate-800 bg-slate-950/95 px-5 py-6 backdrop-blur lg:flex">
      ...
      <nav className="mt-8 space-y-1">{primary.map(({ label, href, icon: Icon }) => { const active = href === activeHref; return <Link key={href} href={href} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition ${active ? "bg-cyan-400 text-slate-950" : "text-slate-400 hover:bg-slate-900 hover:text-white"}`}><Icon className="size-5" />{label}</Link>; })}</nav>
      ...
      <nav className="mt-2 grid min-h-0 flex-1 gap-1 overflow-y-auto pr-1">{secondary.map(([label, href]) => { const active = href === activeHref; return <Link key={href} href={href} className={`rounded-xl px-3 py-2 text-sm transition ${active ? "bg-slate-800 text-cyan-300" : "text-slate-400 hover:bg-slate-900 hover:text-white"}`}>{label}</Link>; })}</nav>
