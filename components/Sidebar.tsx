  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 flex-col border-r border-slate-800 bg-slate-950/95 px-5 py-6 backdrop-blur lg:flex">
      <div className="flex items-center gap-3 px-2">
        ...
        <div><div className="flex items-center gap-2"><p className="text-xl font-black tracking-tight text-white">JR OS</p><span className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-cyan-300">Beta</span></div><p className="text-xs text-slate-500">v0.1 · Internal business suite</p></div>
      </div>
      ...
      <nav className="mt-2 grid gap-1 overflow-y-auto pr-1">
        {secondaryNavigation.map(([label, href]) => {
          const active = pathname === href || (href !== "/" && pathname.startsWith(`${href}/`));
          return <Link key={href} href={href} className={`rounded-xl px-3 py-2 text-sm transition ${active ? "bg-slate-800 text-cyan-300" : "text-slate-400 hover:bg-slate-900 hover:text-white"}`}>{label}</Link>;
        })}
      </nav>
