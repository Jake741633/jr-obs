-- QA inspections contain checklist results, defect notes and inspector detail.
-- Scope the field projection to active jobs assigned to the authenticated
-- electrician so another same-tenant job cannot enter the field UI or cache.

-- JobQaInspection payloads carry jobId but no customerId, so generic office
-- and migrated records legitimately retain a NULL customer envelope. A
-- non-NULL customer remains an exact constraint while the canonical active job
-- and live field assignment are always required.
...
revoke execute on function private.jr_field_qa_inspection_targets_assigned_job(uuid, text, text)
from public, anon, authenticated, service_role;
grant execute on function private.jr_field_qa_inspection_targets_assigned_job(uuid, text, text)
...
as $$
  select pg_catalog.jsonb_build_object(
    'migration',
    '20260826120037_scope_field_job_qa_reads_to_assignments.sql'
  )
$$;
