-- Electrician timeline writes are already bound to an assigned canonical job,
-- but the field projection still exposed every same-tenant timeline row. Scope
-- timeline reads to the same active assignment while preserving the existing
-- survey boundary and all other field collection policies for separate review.

-- Production JobTimelineEntry payloads carry jobId but not customerId, so the
-- relational customer envelope can legitimately be NULL. Treat a non-NULL
-- customer as an exact constraint, then resolve assignment through the active
-- canonical job and unique field identity.
...
revoke execute on function private.jr_field_timeline_targets_assigned_job(uuid, text, text)
from public, anon, authenticated, service_role;
grant execute on function private.jr_field_timeline_targets_assigned_job(uuid, text, text)
...
as $$
  select pg_catalog.jsonb_build_object(
    'migration',
    '20260820160000_scope_field_timeline_reads_to_assignments.sql'
  )
$$;
