-- Generic office and migrated JobProgress rows legitimately retain a NULL
-- customer envelope because their browser payload has jobId but no customerId.
-- The assigned-job read policy accepts that production shape, so the dedicated
-- field update RPC must accept it too while still rejecting any wrong non-NULL
-- customer binding.
...
as $$
  select pg_catalog.jsonb_build_object(
    'migration',
    '20260826112805_align_field_progress_update_customer_envelopes.sql'
  )
$$;
