-- Variation pricing is redacted from the field projection, but the SELECT
-- policy still exposes same-tenant variations for jobs outside the reader's
-- assignment. Bind variation reads to the active canonical job and unique
-- field identity before the projected payload reaches the browser or cache.

-- JobVariation payloads carry jobId but not customerId, so the relational
-- customer envelope can legitimately be NULL. Treat a non-NULL customer as
-- an exact constraint while requiring an active assigned canonical job.
...
revoke execute on function private.jr_field_variation_targets_assigned_job(uuid, text, text)
from public, anon, authenticated, service_role;
grant execute on function private.jr_field_variation_targets_assigned_job(uuid, text, text)
...
as $$
  select pg_catalog.jsonb_build_object(
    'migration',
    '20260826101908_scope_field_variation_reads_to_assignments.sql'
  )
$$;
