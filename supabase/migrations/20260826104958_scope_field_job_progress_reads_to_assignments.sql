-- Job-progress projections remain operationally useful to assigned engineers,
-- but the previous SELECT policy exposed every same-tenant progress record.
-- Bind reads to the active field identity and canonical assigned job before a
-- progress payload reaches the browser or its offline cache.

-- JobProgress payloads carry jobId but no customerId, so generic office and
-- migrated records can legitimately have a NULL customer envelope. Treat a
-- non-NULL customer as an exact constraint while always requiring an active,
...
revoke execute on function private.jr_field_progress_targets_assigned_job(uuid, text, text)
from public, anon, authenticated, service_role;
grant execute on function private.jr_field_progress_targets_assigned_job(uuid, text, text)
...
as $$
  select pg_catalog.jsonb_build_object(
    'migration',
    '20260826104958_scope_field_job_progress_reads_to_assignments.sql'
  )
$$;
