...
export function removeStaffMember(actorProfile, targetProfile, now = new Date().toISOString()) {
  assertManager(actorProfile);
  if (!canManageOrganisationMember(actorProfile, targetProfile)) {
    throw new Error("This staff member cannot be removed by the signed-in account.");
  }
  if (targetProfile?.status === "removed" && targetProfile?.active !== true) {
    return { ...targetProfile };
  }
...
export function reactivateStaffMember(actorProfile, targetProfile, now = new Date().toISOString()) {
  ...
  if (targetProfile?.status === "removed" || targetProfile?.removedAt || targetProfile?.removed_at) {
    throw new Error("Removed staff memberships cannot be reactivated.");
  }
