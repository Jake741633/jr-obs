...
export function createCollectionRepository<T extends RepositoryRecord>(options: {
  ...
  const { storageKey, table, organisationId } = options;
  ...
  return {
    ...
    async list(): Promise<T[]> {
      ...
      try {
        const rows = await cloudSelect<CloudEnvelope<T>>(table, `select=*&organisation_id=eq.${organisationId}&deleted_at=is.null`);
    ...
    save(record: T, expectedVersion?: number) {
      ...
      queueChange({ table, operation: "upsert", organisationId, sourceId: record.id, payload: record, expectedVersion });
    },
    ...
    remove(sourceId: string, expectedVersion?: number) {
      ...
      if (effectiveCloudMode() === "local") return;
      queueChange({ table, operation: "delete", organisationId, sourceId, expectedVersion });
    },
  };
}
