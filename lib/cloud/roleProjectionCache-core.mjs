...
export function customerProjectionCachePolicy({ storageKey, role, mode, generation }) {
  if (role !== "customer" || mode === "local") return "keep";
  if (CUSTOMER_ALWAYS_PURGED_STORAGE_KEYS.has(storageKey)) return "purge";
  if (CUSTOMER_PROJECTION_STORAGE_KEYS.has(storageKey)
    && generation !== CUSTOMER_PROJECTION_CACHE_GENERATION) return "purge";
  return "keep";
}
