...
const STAFF_ASSIGNABLE_ROLES = ["admin", "office", "electrician", "customer"];
...
function assertInviteNotExpired(invite, now) {
  ...
  if (!Number.isFinite(expiryTime) || !Number.isFinite(acceptedTime)) {
    throw new Error("This staff invitation does not contain a valid expiry window.");
  }
  if (acceptedTime >= expiryTime) {
...
export function linkAcceptedStaffInvite(invite, userId, email, now = new Date().toISOString()) {
  ...
  if (!STAFF_ASSIGNABLE_ROLES.includes(invite.role)) throw new Error("This staff invitation contains an invalid role.");
