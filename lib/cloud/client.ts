...
function headers(session?: CloudSession) {
  return {
    ...
    Authorization: `Bearer ${session?.accessToken || cloudConfig.anonKey}`,
...
export const cloudSession = {
  load(): CloudSession | null {
    ...
    try { return JSON.parse(window.localStorage.getItem(SESSION_KEY) || "null") as CloudSession | null; } catch { return null; }
  },
...
export async function signInWithPassword(email: string, password: string) {
  ...
  cloudSession.save(session);
  return session;
}

export async function signOut() {
  const session = cloudSession.load();
  if (session) await request<void>("/auth/v1/logout", { method: "POST" }, session).catch(() => undefined);
...
export async function refreshSession(session = cloudSession.load()) {
  ...
  cloudSession.save(refreshed);
  return refreshed;
}

export async function cloudSelect<T>(table: string, query = "select=*") {
  return request<T[]>(`/rest/v1/${encodeURIComponent(table)}?${query}`, { method: "GET" }, cloudSession.load() || undefined);
}

export async function cloudUpsert<T extends object>(table: string, records: T[]) {
  return request<T[]>(`/rest/v1/${encodeURIComponent(table)}?on_conflict=business_id,source_id`, { method: "POST", body: JSON.stringify(records), headers: { Prefer: "resolution=merge-duplicates,return=representation" } }, cloudSession.load() || undefined);
}

export async function cloudDelete(table: string, sourceId: string) {
  return request<void>(`/rest/v1/${encodeURIComponent(table)}?source_id=eq.${encodeURIComponent(sourceId)}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }, cloudSession.load() || undefined);
}

export async function createSignedUpload(path: string, expiresIn = 600) {
  const session = cloudSession.load();
  return request<{ signedURL: string; token: string }>(`/storage/v1/object/upload/sign/jr-os-private/${encodeURIComponent(path)}`, { method: "POST", body: JSON.stringify({ expiresIn }) }, session || undefined);
}

export async function createSignedDownload(path: string, expiresIn = 300) {
  const session = cloudSession.load();
  return request<{ signedURL: string }>(`/storage/v1/object/sign/jr-os-private/${encodeURIComponent(path)}`, { method: "POST", body: JSON.stringify({ expiresIn }) }, session || undefined);
}
