...
export function roomEstimateFinancials(room, priceBookItems = []) {
  ...
  return selections.reduce((summary, selection) => {
    ...
    summary.lines.push({
      ...
      unitSellingPrice: selection.financials.unit.sellingPrice,
...
export function roomEstimateToQuoteLines(room, priceBookItems = [], makeId = (value) => value) {
  const summary = roomEstimateFinancials(room, priceBookItems);
  return summary.lines.map((line, index) => ({
    id: cleanText(makeId(`${summary.room.id || "room"}-${line.roomPointId || index + 1}`)),
    description: `${summary.room.name}: ${line.description}${line.notes ? ` — ${line.notes}` : ""}`,
    category: "Other",
    quantity: line.quantity,
    unit: line.unitLabel || "point",
    unitCost: 0,
    unitPrice: line.unitSellingPrice,
    priceBookItemId: line.priceBookItemId,
    internalNotes: `Room estimate: ${summary.room.name}${summary.room.internalNotes ? `. ${summary.room.internalNotes}` : ""}`,
  }));
}
...
export function wholePropertyEstimateToQuoteLines(rooms, priceBookItems = [], makeId = (value) => value) {
  return (Array.isArray(rooms) ? rooms : []).flatMap((room, roomIndex) =>
    roomEstimateToQuoteLines(room, priceBookItems, (value) => makeId(`${roomIndex + 1}-${value}`)),
  );
}
