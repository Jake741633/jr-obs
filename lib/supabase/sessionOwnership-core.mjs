...
export function supabaseSessionUserId(session) {
  ...
  if (typeof accessToken === "string") {
    ...
    if (parts.length === 3 && parts[1]) {
      try {
        ...
        // Fall back to the canonical embedded user for legacy/non-JWT fixtures.
      }
    }
  }
  return normalisedSessionUserId(session);
}
...
export function globalSupabaseSignOutOwnsSession(
  ...
  const capturedUserId = typeof expectedUserId === "string" && expectedUserId.trim()
    ? expectedUserId.trim()
    : supabaseSessionUserId(expectedSession);
  return Boolean(capturedUserId && supabaseSessionUserId(currentSession) === capturedUserId);
}
